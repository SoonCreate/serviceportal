...
//	}
	...
	function index() {
		...
		if ( $this->_configured() ) {
			...
			if ( $this->input->get_post('SaveLang') || $this->input->get_post('ConfirmSaveLang') ) {
			...
			if ( $this->validated && $this->input->get_post('ConfirmSaveLang') ) {
				if ( $this->_translation_saved( $result ) ) {
                    custz_message('I','Changes Saved.');
				} else {
					custz_message('E','The file could not be saved.Please contact the system administrator.');
				}
			}
            //不用确认直接修改
//			elseif ( $this->validated &&  $this->input->get_post('SaveLang') ) {
//				$this->data['page_content'] = 'translator/save_lang';
//				$this->data['page_title'] = label('confirm');
//			}
			else {
				$this->data['page_content'] = 'translator/translate_lang';
				$this->data['page_title'] = label('translate'). ' ' . $this->langModule;
			}

		}
        if($_GET){
            render_view('translator/container', $this->data);
        }
	}
	...
	function _configure() {
		...
		foreach ( $this->config_fields as $field ) {
			if ( $this->$field ) {
			} elseif ( mb_strlen( $this->input->get_post( $field ) ) > 0 ) {
				$this->$field = $this->input->get_post( $field );
	...
	function _configured() {
		...
		if ( ! $this->langDir ) {
			...
			$this->data['page_content'] = 'translator/select_lang_directory';
			$this->data['page_title'] = label('select_language_directory');
		}		
		elseif ( ! $this->masterLang ) {
			$this->data['languages'] = $this->ListLanguages();
			$this->data['page_content'] = 'translator/select_master_lang';
			$this->data['page_title'] = label('select_master_language');
		}		
		elseif ( ! $this->input->get_post('slaveLang') ) {
			// $this->data['languages'] = $this->_list_languages( $this->masterLang );
			$this->data['languages'] = $this->_list_languages();
			$this->data['page_content'] = 'translator/select_slave_lang';
			$this->data['page_title'] = label('select_slave_language');
		}		
		elseif ( ! $this->langModule ) {
			...
			$this->data['page_content'] = 'translator/select_module';
			$this->data['page_title'] = label('select_module_to_translate');
		}		
